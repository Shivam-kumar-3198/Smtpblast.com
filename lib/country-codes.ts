/**
 * ITU-T E.164 calling codes for every ISO 3166-1 country/territory, plus
 * the emoji flag (computed from the ISO2 code via Unicode regional
 * indicator symbols, so there's no chance of a mistyped flag).
 */
...
const RAW_COUNTRY_CODES: Omit<CountryCode, "flag">[] = [
...
export const COUNTRY_CODES: CountryCode[] = RAW_COUNTRY_CODES.map((c) => ({
  ...c,
  flag: flagFromIso2(c.iso2),
}));

export function countryOptionLabel(c: CountryCode): string {
